...
export function calculateProductionTime(
  ...
  interface OpSpan { opId: string; startMs: number; endMs: number; lineId?: string }
  ...
  for (const [opId, opEvents] of eventsByOp.entries()) {
    ...
    const groupKey = `${op?.setor || 'geral'}|${op?.scheduledShift || 'turno'}|${dayKey}`;
    const list = spansByGroup.get(groupKey) || [];
    list.push({ opId, startMs, endMs, lineId: op?.lineId || undefined });
  ...
  for (const spans of spansByGroup.values()) {
    ...
    const merged: { startMs: number; endMs: number; lineId?: string }[] = [];
    for (const s of spans) {
      ...
      if (last && s.startMs <= last.endMs) {
        ...
        merged.push({ startMs: s.startMs, endMs: s.endMs, lineId: s.lineId });
      }
    }
